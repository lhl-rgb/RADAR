...
namespace radar
{
  namespace
  {
    void FillMetricsMessage(const core::SimulationMetrics &metrics, SimMetrics *reply)
    {
      reply->set_current_scan(metrics.current_scan);
      reply->set_current_pulse(metrics.current_pulse);
      reply->set_total_pulses(metrics.total_pulses);
      reply->set_progress_percent(metrics.progress_percent);
      reply->set_avg_us_per_prt(metrics.avg_us_per_prt);
      reply->set_realtime_ratio(metrics.realtime_ratio);
      reply->set_target_us(metrics.target_us);
      reply->set_clutter_us(metrics.clutter_us);
      reply->set_noise_us(metrics.noise_us);
      reply->set_compose_us(metrics.compose_us);
      reply->set_output_us(metrics.output_us);
      reply->set_preview_us(metrics.preview_us);
      reply->set_frames_output(metrics.frames_output);
      reply->set_dropped_preview_frames(metrics.dropped_preview_frames);
    }

    void FillRangeProfileMessage(const preview::RangeProfilePreview &profile,
                                 RangeProfilePreviewMsg *reply)
    {
      reply->set_pulse_index(profile.pulse_index);
      reply->set_beam_az_deg(profile.beam_az_deg);
      auto *values = reply->mutable_power_db();
      values->Reserve(static_cast<int>(profile.power_db.size()));
      for (const float value : profile.power_db)
      {
        values->Add(value);
      }
    }

    void FillPpiPreviewMessage(const preview::PpiPreviewFrame &frame,
                               PpiPreviewMsg *reply)
    {
      reply->set_scan_index(frame.scan_index);
      reply->set_range_bins(frame.range_bins);
      reply->set_az_bins(frame.az_bins);
      reply->set_dynamic_range_db(frame.dynamic_range_db);
      if (!frame.image.empty())
      {
        reply->set_image(reinterpret_cast<const char *>(frame.image.data()),
                         frame.image.size());
      }
    }
  } // namespace
                                                 ...
                                                 Status *reply)
  {
    ...
    if (request->peak_power_w() > 0)
    {
      current_config_.system.peak_power_w = request->peak_power_w();
    }
                                                 ...
                                                 SystemParams *reply)
  {
    ...
    reply->set_peak_power_w(current_config_.system.peak_power_w);
    return grpc::Status::OK;
  }
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    current_config_.antenna.az_beamwidth_deg = request->az_beamwidth_deg();
    current_config_.antenna.el_beamwidth_deg = request->el_beamwidth_deg();
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    reply->set_az_beamwidth_deg(current_config_.antenna.az_beamwidth_deg);
    reply->set_el_beamwidth_deg(current_config_.antenna.el_beamwidth_deg);
    reply->set_peak_gain_db(current_config_.antenna.peak_gain_db);
    return grpc::Status::OK;
  }

  grpc::Status RadarServiceImpl::SetMechScanParams(grpc::ServerContext *,
                                                   const MechScanParams *request,
                                                   Status *reply)
  {
    std::lock_guard<std::mutex> lock(mutex_);
    current_config_.mech_scan.rotation_rate_dps = request->rotation_rate_dps();
    current_config_.mech_scan.az_start_deg = request->az_start_deg();
    current_config_.mech_scan.az_end_deg = request->az_end_deg();
    current_config_.mech_scan.elevation_deg = request->elevation_deg();
    current_config_.compute_derived_params();
    reply->set_success(true);
    reply->set_message("Mechanical scan params updated");
    return grpc::Status::OK;
  }

  grpc::Status RadarServiceImpl::GetMechScanParams(grpc::ServerContext *,
                                                   const Empty *,
                                                   MechScanParams *reply)
  {
    std::lock_guard<std::mutex> lock(mutex_);
    reply->set_rotation_rate_dps(current_config_.mech_scan.rotation_rate_dps);
    reply->set_az_start_deg(current_config_.mech_scan.az_start_deg);
    reply->set_az_end_deg(current_config_.mech_scan.az_end_deg);
    reply->set_elevation_deg(current_config_.mech_scan.elevation_deg);
    return grpc::Status::OK;
  }
                                                  ...
                                                  Status *reply)
  {
    ...
    current_config_.clutter.sea_state = request->sea_state();
    ...
    current_config_.clutter.az_patch_step_deg = request->az_patch_step_deg();
    current_config_.clutter.active_gain_floor_db = request->active_gain_floor_db();
    current_config_.clutter.distribution =
        static_cast<ClutterDistribution>(request->distribution());
    current_config_.clutter.weibull_shape = request->weibull_shape();
    current_config_.clutter.weibull_scale = request->weibull_scale();
    current_config_.clutter.lognormal_mu = request->lognormal_mu();
    current_config_.clutter.lognormal_sigma = request->lognormal_sigma();
    current_config_.clutter.k_shape_nu = request->k_shape_nu();
    current_config_.clutter.k_texture_bandwidth_hz = request->k_texture_bandwidth_hz();
    current_config_.clutter.k_lut_size = request->k_lut_size();
    reply->set_success(true);
                                                  ...
                                                  ClutterParams *reply)
  {
    ...
    reply->set_sea_state(current_config_.clutter.sea_state);
    ...
    reply->set_az_patch_step_deg(current_config_.clutter.az_patch_step_deg);
    reply->set_active_gain_floor_db(current_config_.clutter.active_gain_floor_db);
    reply->set_distribution(
        static_cast<ClutterParams::ClutterDistribution>(
            static_cast<int>(current_config_.clutter.distribution)));
    reply->set_weibull_shape(current_config_.clutter.weibull_shape);
    reply->set_weibull_scale(current_config_.clutter.weibull_scale);
    reply->set_lognormal_mu(current_config_.clutter.lognormal_mu);
    reply->set_lognormal_sigma(current_config_.clutter.lognormal_sigma);
    reply->set_k_shape_nu(current_config_.clutter.k_shape_nu);
    reply->set_k_texture_bandwidth_hz(current_config_.clutter.k_texture_bandwidth_hz);
    reply->set_k_lut_size(current_config_.clutter.k_lut_size);
    return grpc::Status::OK;
  }
                                              ...
                                              Status *reply)
  {
    ...
    auto it = std::remove_if(
        ...
        [id_to_remove](const TargetState &t)
        { return t.id == id_to_remove; });
  ...
  grpc::Status RadarServiceImpl::GetMetrics(grpc::ServerContext *, const Empty *,
                                            SimMetrics *reply)
  {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (!engine_)
    {
      reply->set_total_pulses(static_cast<std::uint64_t>(
          std::max(0, current_config_.simulation.scan_count *
                          current_config_.mech_scan.pulses_per_rotation)));
      return grpc::Status::OK;
    }

    const auto metrics = engine_->metrics();
    FillMetricsMessage(metrics, reply);
    return grpc::Status::OK;
  }

  grpc::Status RadarServiceImpl::StreamMetrics(grpc::ServerContext *context, const Empty *,
                                               grpc::ServerWriter<SimMetrics> *writer)
  {
    ...
    while (!context->IsCancelled())
    {
      SimMetrics reply;
      {
        ...
        if (engine_)
        {
          FillMetricsMessage(engine_->metrics(), &reply);
        }
        else
        {
          reply.set_total_pulses(static_cast<std::uint64_t>(
  ...
  grpc::Status RadarServiceImpl::StreamRangeProfile(
      grpc::ServerContext *context,
      const Empty *,
      grpc::ServerWriter<RangeProfilePreviewMsg> *writer)
  {
    using namespace std::chrono_literals;
    std::uint64_t last_pulse = std::numeric_limits<std::uint64_t>::max();
    ...
    while (!context->IsCancelled())
    {
      ...
      bool has_update = false;
      {
        ...
        if (engine_)
        {
          const auto profile = engine_->latest_range_profile();
          if (profile.has_value() && profile->pulse_index != last_pulse)
          {
            FillRangeProfileMessage(*profile, &reply);
            last_pulse = profile->pulse_index;
            has_update = true;
          }
        }
      }

      if (has_update && !writer->Write(reply))
      {
        break;
      }
  ...
  grpc::Status RadarServiceImpl::StreamPpiPreview(grpc::ServerContext *context, const Empty *,
                                                  grpc::ServerWriter<PpiPreviewMsg> *writer)
  {
    using namespace std::chrono_literals;
    ...
    while (!context->IsCancelled())
    {
      ...
      bool has_frame = false;
      {
        ...
        if (engine_)
        {
          const auto frame = engine_->latest_ppi_frame();
          if (frame.has_value())
          {
            FillPpiPreviewMessage(*frame, &reply);
            has_frame = true;
          }
        }
      }

      if (has_frame && !writer->Write(reply))
      {
        break;
      }
      std::this_thread::sleep_for(200ms);
