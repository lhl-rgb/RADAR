...
namespace
{
    ...
    BenchmarkResult run_path(radar::clutter::SeaClutterEngine &engine,
                             int total_pulses,
                             int pulses_per_rotation,
                             float az_step_per_prt_deg,
                             int mode)
    {
        ...
        for (int i = 0; i < std::min(8, pulses_per_rotation); ++i)
        {
            ...
            if (mode == 2)
            {
                ok = engine.get_clutter_for_pulse_timed_2d_fast(beam, i, echo, &timing);
            }
            else if (mode == 1)
            {
                ok = engine.get_clutter_for_pulse_timed_2d(beam, i, echo, &timing);
            }
            else
            {
                ok = engine.get_clutter_for_pulse_timed(beam, i, echo, &timing);
            }
            if (!ok)
            {
        ...
        for (int pulse = 0; pulse < total_pulses; ++pulse)
        {
            ...
            if (mode == 2)
            {
                ok = engine.get_clutter_for_pulse_timed_2d_fast(beam, pulse, echo, &timing);
            }
            else if (mode == 1)
            {
                ok = engine.get_clutter_for_pulse_timed_2d(beam, pulse, echo, &timing);
            }
            else
            {
                ok = engine.get_clutter_for_pulse_timed(beam, pulse, echo, &timing);
            }
            if (!ok)
            {
                throw std::runtime_error(engine.last_error());
            ...
            result.active_upload_ms_sum += timing.gpu.active_upload_ms;
                      ...
                      double simulated_seconds)
    {
        ...
        const double avg_us_per_prt = (result.run_ms * 1000.0) / pulses_d;
        const double prt_per_sec = 1000.0 * pulses_d / result.run_ms;
        const double realtime_ratio = simulated_seconds / (result.run_ms / 1000.0);
        ...
        std::cout << "  avg_active_upload_us:    " << (result.active_upload_ms_sum * 1000.0 / pulses_d) << "\n";
...
int main(int argc, char **argv)
{
    ...
    {
        std::cout << "CUDA clutter 2D benchmark skipped: " << cuda_error << "\n";
        return 0;
    }
    ...
    radar::clutter::SeaClutterEngine engine_1d;
    engine_1d.set_config(cfg);
    engine_1d.set_antenna_config(ant);

    radar::clutter::SeaClutterEngine engine_2d;
    engine_2d.set_config(cfg);
    engine_2d.set_antenna_config(ant);

    radar::clutter::SeaClutterEngine engine_2d_fast;
    engine_2d_fast.set_config(cfg);
    engine_2d_fast.set_antenna_config(ant);

    const auto init_start = Clock::now();
    if (!engine_1d.initialize(sys))
    {
        std::cerr << "SeaClutterEngine 1D initialize failed: " << engine_1d.last_error() << "\n";
        return 1;
    }
    if (!engine_2d.initialize(sys))
    {
        std::cerr << "SeaClutterEngine 2D initialize failed: " << engine_2d.last_error() << "\n";
        return 1;
    }
    if (!engine_2d_fast.initialize(sys))
    {
        std::cerr << "SeaClutterEngine 2D-fast initialize failed: " << engine_2d_fast.last_error() << "\n";
        return 1;
    }
    ...
    BenchmarkResult one_d;
    BenchmarkResult two_d;
    BenchmarkResult two_d_fast;
    try
    {
        one_d = run_path(engine_1d, total_pulses, pulses_per_rotation, az_step_per_prt_deg, 0);
        two_d = run_path(engine_2d, total_pulses, pulses_per_rotation, az_step_per_prt_deg, 1);
        two_d_fast = run_path(engine_2d_fast, total_pulses, pulses_per_rotation, az_step_per_prt_deg, 2);
    }
    ...
    std::cout << "  init_ms_three_engines:   " << init_ms << "\n";
    print_result("1D range-thread path", one_d, total_pulses, simulated_seconds);
    print_result("2D range_patch path", two_d, total_pulses, simulated_seconds);
    print_result("2D fast range_patch path", two_d_fast, total_pulses, simulated_seconds);

    if (two_d.run_ms > 0.0)
    {
        std::cout << "  speedup_2d_vs_1d:        " << (one_d.run_ms / two_d.run_ms) << "x\n";
    }
    if (two_d_fast.run_ms > 0.0)
    {
        std::cout << "  speedup_2d_fast_vs_1d:   " << (one_d.run_ms / two_d_fast.run_ms) << "x\n";
        std::cout << "  speedup_2d_fast_vs_2d:   " << (two_d.run_ms / two_d_fast.run_ms) << "x\n";
    }
