...
namespace radar::math
{
    ...
    {
        return std::pow(10.0f, db_value / 10.0f);
    }
    ...
    {
        return 10.0f * std::log10(std::max(linear_value, EPSILON));
    }
    ...
    {
        Scalar wrapped = std::fmod(azimuth_deg, 360.0f);
        if (wrapped < -180.0f)
        {
            wrapped += 360.0f;
        }
