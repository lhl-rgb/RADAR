...
namespace radar::waveform
{
    ...
    namespace
    {
        ...
        {
            if (sys.fs_hz <= 0.0f || sys.pulse_width_s <= 0.0f)
            {
    ...
    bool WaveformGenerator::initialize()
    {
        ...
        case WaveformType::CW:
            waveform_.assign(
                static_cast<std::size_t>(num_samples), Complex(1.0f, 0.0f));
            break;
        default:
            waveform_.assign(
                static_cast<std::size_t>(num_samples), Complex(0.0f, 0.0f));
            break;
        }
    ...
    ComplexVec WaveformGenerator::generate_lfm(int num_samples) const
    {
        ...
        const Scalar fs = std::max(sys_.fs_hz, 1.0f);
        const Scalar dt = 1.0f / fs;
        ...
        if (B <= EPSILON)
        {
            std::fill(waveform.begin(), waveform.end(), Complex(1.0f, 0.0f));
            return waveform;
        }
        ...
        for (int n = 0; n < num_samples; ++n)
        {
            ...
            const Scalar phase = 2.0f * PI * (-0.5f * B * t + 0.5f * chirp_rate * t * t);
            waveform[static_cast<std::size_t>(n)] = std::polar(1.0f, phase);
        }
    ...
    ComplexVec WaveformGenerator::generate_nlfm(int num_samples) const
    {
        ...
        ScalarVector inst_freq(N_out, 0.0f);

        for (int n = 0; n < num_samples; ++n)
        {
            const Scalar t = sample_time(n, dt); // ★ 统一时间轴
            const Scalar t_clamped = std::clamp(t, 0.0f, T);
        ...
        {
            phase += 2.0f * PI * inst_freq[n] * dt; // dt = 采样间隔
            waveform[n] = std::polar(1.0f, phase);
        }
        ...
        int length, WindowType type) const
    {
        ...
        {
            window[i] = Complex(weights[i], 0.0f);
        }
        return window;
    }
        ...
        int length, WindowType type) const
    {
        ...
        ScalarVector window(static_cast<std::size_t>(length), 1.0f);
        if (length == 1)
            return window;
        ...
        for (int n = 0; n < length; ++n)
        {
            ...
            case WindowType::Hann:
                window[static_cast<std::size_t>(n)] =
                    0.5f * (1.0f - std::cos(2.0f * PI * x));
                break;
            case WindowType::Hamming:
                window[static_cast<std::size_t>(n)] =
                    0.54f - 0.46f * std::cos(2.0f * PI * x);
                break;
            case WindowType::Blackman:
                ...
                break;
            default: // Rectangular
                window[static_cast<std::size_t>(n)] = 1.0f;
                break;
            }
        }
        return window;
